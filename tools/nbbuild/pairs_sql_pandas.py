...
код здесь пишется так, чтобы его можно было читать глазами, а не только
...
PAIRS = [
    dict(
        ...
        why="Первый вопрос к незнакомым данным: сколько строк и за какой период. "
            "Пока это не проверено, любая цифра ниже повисает в воздухе.",
...
FROM raw_applications""",
    ...
    dict(
        ...
        why="Тип столбца решает, что с ним можно делать. Сумма, приехавшая "
            "текстом, тихо ломает среднее.",
...
ORDER BY ordinal_position""",
    ...
    dict(
        ...
        why="Глазами по десятку строк видно то, чего не видно ни в одной "
...
ORDER BY канал""",
    ...
    dict(
        ...
        why="Самая частая операция разведочного анализа. Показывает и "
            "структуру данных, и мусор в них.",
...
справа = pd.DataFrame([{"сумма": round(суммы.sum())}])""",
        note="`errors=\"coerce\"` превращает неразобранное в `NaN` вместо "
             ...
             "были мусором. В базе тип задан схемой, приводить нечего.",
    ),
    dict(
        n=11,
